...
## 5.5 Agent Loops — the engine of smarter, more sophisticated design

The linear pipeline above ships *a* result. **Loops** are how the result gets *better than any single pass could produce.* Each loop is a small, bounded control structure with an explicit exit condition — never an open-ended "keep going." Five loop archetypes drive the design system; a set-piece composes the ones it needs.

### L1 — Refine-until-bar (quality convergence)
Build → self-critique against the §7 Quality Bar → patch → re-verify, repeating until the bar passes or `K` iterations elapse. Design is iterative by nature; the first render of the Observatory is never the shippable one. The loop makes "good" a *destination*, not a lucky first draft.
```
let pass = false, i = 0
while (!pass && i++ < K) {
  build/refine()
  const report = verify(QualityBar)     // perf, a11y, fallback, "awe test"
  pass = report.allGreen
  if (!pass) applyFixes(report.failures)
}
```

### L2 — Judge-panel tournament (taste convergence)
For any high-variance creative call (silhouette dispersion math, camera easing, bloom curve), generate **N diverse variants in parallel**, score each with an independent **judge panel** against explicit criteria + reference frames, keep the winner, and **graft the best idea from each runner-up**. Taste is comparative — you cannot know a motion is right until you have felt three wrong ones next to it.
```
const variants = await parallel(range(N).map(seed => build(seed)))
const scored   = await parallel(variants.map(v => judgePanel(v, criteria)))
const winner   = best(scored)
return synthesize(winner, graftHighlights(runnersUp(scored)))
```

### L3 — Adversarial verify loop (trust convergence)
Every finding/render is handed to **independent skeptics**, each prompted to *break* it from a distinct lens (perf, a11y, cross-device, visual regression, motion sickness). A finding survives only on a **majority pass**; survivors that fail re-enter L1. This is what stops a plausible-but-wrong effect from shipping.

### L4 — Loop-until-dry (completeness convergence)
For open-ended discovery — "what's missing from this scene?", "which edge devices break?", "what accessibility gaps remain?" — keep spawning critics until **`M` consecutive rounds surface nothing new**, deduping against everything already seen. Simple "run once" checks miss the tail; the dry-streak counter finds it.

### L5 — Budget-scaled depth (resource convergence)
Loop depth **scales to the remaining token budget**. Cheap phases run 1–2 refinement passes; the hero and visualizer earn 4–5 pass tournaments. The loop reads `budget.remaining()` and stops enriching when the pool is spent — the 2M budget is spread by *impact*, not evenly.
```
while (budget.remaining() > FLOOR && !converged) { refineOrExpand() }
```

### Composition & guardrails
- **Nesting:** a set-piece typically runs **L2 (variants) → L3 (adversarial verify each) → L1 (refine the winner to the bar)**, all bounded by **L5**.
- **Every loop declares its exit up front** — `K` passes, `M` dry rounds, or budget floor. No loop runs unbounded; the roadmap's per-phase token cap is the hard ceiling.
- **Convergence is logged**, not assumed: each iteration records what changed and why it did/didn't pass, so a human can audit the taste, not just the outcome.
- **Human gate is the outermost loop:** a phase's loops converge → PR → *your approval* re-enters the outer loop for the next phase. The machine tightens; the human directs.

> Loops are the difference between *"an agent built the hero"* and *"the hero is the best of five adversarially-verified attempts, refined to a measured 60fps bar, within budget."* That gap is the sophistication.
