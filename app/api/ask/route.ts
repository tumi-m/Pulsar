...
export async function POST(req: NextRequest) {
  ...
  if (!text) return NextResponse.json({ moods: [], genres: [], decades: [], freeText: "" });
  if (!BASE_URL || !API_KEY) {
    // No cloud model configured — caller falls back to keyword parsing.
    return NextResponse.json({ moods: [], genres: [], decades: [], freeText: text });
  }
  ...
  try {
    ...
    if (!res.ok) return NextResponse.json({ moods: [], genres: [], decades: [], freeText: text });
    const data = await res.json();
    const raw = data?.message?.content;
    const match = typeof raw === "string" ? raw.match(/\{[\s\S]*\}/) : null;
    if (!match) return NextResponse.json({ moods: [], genres: [], decades: [], freeText: text });
